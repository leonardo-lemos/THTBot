...
/**
 * This class implements a Temperature Monitor by extending the Monitor class.
 *
...
public class TemperatureMonitor extends Monitor {
    ...
    /**
     * Creates a new instance of a Temperature Monitor.
     *
     * @param minLimit The minimum temperature value to be used for alerts.
     * @param maxLimit The maximum temperature value to be used for alerts.
     * @param botToken The Telegram's bot token to send messages.
     * @param chatID The Telegram's chat ID to send messages.
     * @since 1.0
     */
    public TemperatureMonitor(float minLimit, float maxLimit, String botToken,
            String chatID) {
    ...
    public void update(float temp, float humi, float tens) {
        ...
        if (lastAlertDate == null || TimeUtil.getHoursElapsed(lastAlertDate,
            ...
            if (temp < minLimit) {
                sendMessage("A tempetura do datacenter do CT-6 está abaixo do limite"
                        + " (Temperatura atual: " + temp + ").");
            } else if (temp > maxLimit) {
                sendMessage("A tempetura do datacenter do CT-6 está acima do limite"
                        + " (Temperatura atual: " + temp + ").");
            }
        }
    }
