...
/**
 * This class implements a Tension Monitor by extending the Monitor class.
 *
 * @author Leonardo Lemos
 * @version 1.0
 */
public class TensionMonitor extends Monitor {

    /**
     * Creates a new instance of a Tension Monitor.
     *
     * @param minLimit The minimum tension value to be used for alerts.
     * @param maxLimit The maximum tension value to be used for alerts.
     * @param botToken The Telegram's bot token to send messages.
     * @param chatID The Telegram's chat ID to send messages.
     * @since 1.0
     */
    public TensionMonitor(float minLimit, float maxLimit, String botToken,
            String chatID) {
    ...
    public void update(float temp, float humi, float tens) {
        ...
        if (lastAlertDate == null || TimeUtil.getHoursElapsed(lastAlertDate,
            ...
            if (tens < minLimit) {
                sendMessage("A tensão do datacenter do CT-6 está abaixo do limite"
                        + " (Tensão atual: " + tens + ").");
            } else if (tens > maxLimit) {
                sendMessage("A tensão do datacenter do CT-6 está acima do limite"
                        + " (Tensão atual: " + tens + ").");
            }
        }
    }
